...
export const loginController: APIController<{
  session: string;
  userId: string;
}> = async (req, res, _next) => {
  try {
    ...
    const validateLogin = UserSchema.pick({
      studentid: true,
      password: true,
    }).safeParse(body);
    ...
    if (!validateLogin.success)
      return res
        .status(403)
        .json({ error: { zodError: validateLogin.error.format() } });
...
export const signoutController: APIController<string> = async (
  _req,
  res,
  _next,
) => {
...
export const registerController: APIController<string> = async (
  req,
  res,
  _next,
) => {
  try {
    const { confirmPassword, ...restbody } = decrypt<
      UserFormType & { confirmPassword: string }
    >(req.body.data);
    ...
    if (restbody.password !== confirmPassword)
...
export const changePasswordController: APIController<string> = async (
  req,
  res,
  _next,
) => {
  try {
    ...
    const validateChangePassword = UserSchema.pick({
      studentid: true,
      password: true,
    }).safeParse(body);

    if (!validateChangePassword.success)
      return res
        .status(400)
        .json({ error: { zodError: validateChangePassword.error.format() } });
...
export const checkPasswordController: APIController<string> = async (
  req,
  res,
  _next,
) => {
...
export const changePasswordValidTicketController: APIController<User> = async (
  req,
  res,
  _next,
) => {
  ...
  try {
    ...
    const user = await prisma.user.findUnique({
      where: { studentid: validTicket.studentid },
    });
...
export const generateChangePasswordTicketController: APIController<
  string,
...
> = async (req, res, _next) => {
  ...
  try {
    ...
    const emailParams = new EmailParams()
      ...
      .setHtml(
        `Your ticket is <a href="${env.CLIENT_DOMAIN}/change-password/${ticket}">here</a>`,
      )
