...
export async function findCommunity(query: Partial<Omit<Communities, "createdAt">>): Promise<Communities[]> {
    return await prisma.communities.findMany({where: query});
}

export async function findCommunitybyId(id: string): Promise<Communities | null> {
